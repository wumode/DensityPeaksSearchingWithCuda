...
% for i=r+1:m-r
%     for j=r+1:n-r
%        diffI = abs(I_in(i-r:i+r, j-r:j+r)-I_in(i,j));
%        c = zeros(1,r);
%        for l = 1:r
%            diffI(r-l+2:r+l, r-l+2:r+l) = maxGray+1e-5;
%            [vmin_col, imin_col]=min(diffI(r-l+1:r+l+1, r-l+1:r+l+1));
%            [vmin, imin_row] = min(vmin_col);
%            diffmin_col = imin_row;
%            diffmin_row = imin_col(imin_row);
%            ix = i-l+diffmin_row-1;
%            iy = j-l+diffmin_col-1;
%            c(l) = I_in(i,j)- I_in(ix,iy);
%        end
%        c_abs = abs(c);
%        S(i,j) = dot(c_abs, c);
%        S(i,j) = max( 0,S(i,j));
        ...
        for j = 2: numColumns+1
            ...
            if (pxC ~= 0 && pyC ~= 0)   % find the cluster point. 
                ...
                if ( rhoPoint < maxRho )    % centre point not local maximum.
                    ...
                    % delta( (pyC-1)*nRows + pxC ) = distCandidate(pos)*sqrt(S(pxC, pyC));
...
%                                 sqrt( (pxN-pxC)^2+(pyN-pyC)^2 )*S(pxC, pyC);
                            delta( (pyC-1)*nRows + pxC ) = ...
    ...
    deltaMat = reshape(delta, nRows, nColumns);
...
%     axis equal;
%     hold on;
%     [signRows, signCols] = find(signMat);
%     plot(signCols, signRows, 'ro');
%     
%     hold off;
%     pause(0.1);
